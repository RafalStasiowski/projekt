class Current_Order
  ...
  def update_order(order, args)
    ...
    @status = args[:status] || "Oczekuje"
  end

  def paypal_url(current_user, return_url)
    @user = current_user
		values = {
			:business => 'sb-eibvx672369@business.example.com',
			:cmd => '_cart',
			:upload => 1,
      :rm => 2,
			:return => return_url,
      ...
      "item_name_#{@ordered_items.length+2}" => " Koszty wysyłki"
		}
    counter = 1
  ...
  def add_extra_time(pick_up_time)
    if (Order.first.Status != "Zakończone") || (Order.first.Status == "Anulowane")
       pick_up_time + 4
     else 
      pick_up_time
    end
